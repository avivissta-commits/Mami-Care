...
function buildRoutine() {
  ...
  if (hasRetinol && !hasAcids) {
    warnings.push("התחל 2-3 פעמים בשבוע בלילה. קרם הגנה חובה למחרת. אם יש גירוי - הפסק ל-2-3 ימים.");
  }
...
function renderRecommendationsSection() {
  ...
  const recommended = recommendedProducts[Math.floor(Math.random() * recommendedProducts.length)] || products[0];
  ...
  return el("div", { className: "card recommendations-section" }, [
    el("div", { className: "card-inner", style: "padding-top: 1.25rem; padding-right: 1rem;" }, [
      el("h3", { text: "המלצות עבורך" }),
      el("p", { className: "section-desc", style: "color: #9075D7;", text: "מוצר מומלץ לבניית שגרה" }),
      renderProductCard(recommended, { hideBorder: true, benefitText }),
    ]),
  ]);
}
  ...
  const openSavedRoutine = () => {
    ...
    if (saved.context) {
      setState({
        ...
        routineType: saved.context.routineType || state.routineType,
...
function renderProductCard(product, options = {}) {
  ...
  const { hideBorder = false, benefitText = null } = options;
  ...
  return el("article", { 
    ...
    el("button", { className: `favorite-button card-favorite ${isFavorite ? "active" : ""} ${isFavoritePulsing ? "pulse" : ""}`, "aria-label": "מועדפים", "aria-pressed": String(isFavorite), text: isFavorite ? "♥" : "♡", onClick: (event) => { event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); toggleFavorite(product.id, event.currentTarget); } }),
